...
interface StockOutUploadModalProps {
	open: boolean;
	onCancel: () => void;
	onSuccess: () => void;
}
...
const StockOutUploadModal: React.FC<StockOutUploadModalProps> = ({ open, onCancel, onSuccess }) => {
	...
	const [loadingData, setLoadingData] = useState(false); // 加载产品数据的状态
	...
	const columns: TableProps<StockOutRecord>["columns"] = [
		{
			title: "行号",
			key: "rowIndex",
			width: 60,
	...
	const parseExcelFile = async (file: File): Promise<StockOutRecord[]> => {
		return new Promise((resolve, reject) => {
			...
			reader.onload = e => {
				try {
					...
					// 验证必需的字段是否存在
					const requiredFields = ["productId", "count", "price"];
					...
					for (let i = 2; i < jsonData.length; i++) {
						...
						const price = Number(row[fieldMap["price"]]);

						// 验证数据有效性
						if (!isNaN(productId) && !isNaN(count) && !isNaN(price)) {
							records.push({
								productId,
								count,
								price,
	...
	const customRequest = async (options: any) => {
		...
		try {
			...
			// 前端解析 Excel 文件
			const records: StockOutRecord[] = await parseExcelFile(file as File);
	...
	const handleConfirmImport = async () => {
		try {
			...
			// 将 StockOutRecord[] 转换为 IProductJoinStockOut[] 格式
			const productJoinStockOut: IProductJoinStockOut[] = parsedRecords.map(record => ({
			...
			if (res.code === 200) {
				message.success(`成功导入 ${parsedRecords.length} 条出货记录`);
				// 上传成功后刷新列表
				onSuccess();
				// 重置状态并关闭弹窗
				handleModalCancel();
			...
			{currentStep === 0 && (
				<Dragger {...props} disabled={uploading}>
					...
					<p className="ant-upload-hint">
						...
						<br />
						注意：第一行将被忽略，第二行应为字段名称（productId, count, price），第三行开始为数据
					</p>
				</Dragger>
			)}
...
export default StockOutUploadModal;
